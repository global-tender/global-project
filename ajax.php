...
{?>
	...
	if (isset($_POST['gp_name']) && isset($_POST['gp_email']) && isset($_POST['gp_message']))
	{
		...
		/* ========== */
		$content = "Вам отправлен вопрос с сайта проект-планировки-территории.рус.\n\nИмя: {$name}\nТелефон: {$phone}\nE-Mail: {$email}\nСообщение:\n\n{$message}\n\n\nСообщение сформировано автоматически.\n";
		$from_user = '=?UTF-8?B?'.base64_encode('Проект Планировки Территории').'?=';
		$subject = '=?UTF-8?B?'.base64_encode('Задан вопрос на сайте проект-планировки-территории.рус').'?=';
...
elseif (isset($_GET['get_price'])) {
	...
	if (isset($_POST['gp_email']) && isset($_POST['gp_square']) && isset($_POST['gp_type']) && isset($_POST['gp_phone']))
	{
		...
		/* ========== */
		$content = "Заявка Рассчитать Стоимость с сайта проект-планировки-территории.рус.\n\nE-Mail: {$email}\nТелефон: {$phone}\nПлощадь: {$ob_square}\nВид объекта: {$ob_type}\n\nСообщение сформировано автоматически.\n";
		$from_user = '=?UTF-8?B?'.base64_encode('Проект Планировки Территории').'?=';
		$subject = '=?UTF-8?B?'.base64_encode('Заявка Рассчитать Стоимость с сайта проект-планировки-территории.рус').'?=';
...
elseif (isset($_GET['order_call'])) {
	...
	if (isset($_POST['gp_name']) && isset($_POST['gp_phone']))
	{
		...
		/* ========== */
		$content = "Заявка перезвонить с сайта проект-планировки-территории.рус.\n\nИмя: {$name}\nТелефон: {$phone}\n\n\nСообщение сформировано автоматически.\n";
		$from_user = '=?UTF-8?B?'.base64_encode('Проект Планировки Территории').'?=';
		$subject = '=?UTF-8?B?'.base64_encode('Заявка перезвонить с сайта проект-планировки-территории.рус').'?=';
